...
from transformers_zamba2.activations import ACT2FN
from transformers_zamba2.cache_utils import Cache, DynamicCache
from transformers_zamba2.modeling_attn_mask_utils import (
    AttentionMaskConverter,
)
from transformers_zamba2.modeling_outputs import (
    BaseModelOutputWithPast,
    CausalLMOutputWithPast,
    SequenceClassifierOutputWithPast,
)
from transformers_zamba2.modeling_utils import PreTrainedModel
from transformers_zamba2.utils import (
...
from transformers_zamba2.utils.import_utils import (
    is_causal_conv1d_available,
    is_flash_attn_2_available,
    is_mamba_ssm_available,
)
from transformers_zamba2.models.zamba2.configuration_zamba2 import Zamba2Config
from transformers_zamba2.models.zamba2.mamba2_layer import Mamba2Layer
...
class Zamba2Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if getattr(self, "zero_last_token", False):
            attn_weights[self.ends[:, 0], :, self.ends[:, 1]] = 0.0
            # attn_weights[self.ends[:, 0].unsqueeze(1), self.heads, self.ends[:, 1].unsqueeze(1)] = 0.0
        # if isinstance(mask, torch.Tensor):
        #     attn_weights = attn_weights * mask[None, None, :, :]
        # elif isinstance(mask, list) and isinstance(mask[0], int):
        #     attn_weights[:, mask] = 0.0
        # elif isinstance(mask, list) and isinstance(mask[0], tuple):
        #     for b, q in mask:
        #         attn_weights[b, :, q, :] = 0.0
        # elif mask == "zeros":
...
class Zamba2AttentionDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states, self_attn_weights, present_key_value = self.self_attn(hidden_states, attention_mask=attention_mask, past_key_value=past_key_value, position_ids=position_ids, layer_idx=layer_idx, output_attentions=output_attentions, mask=mask, use_cache=use_cache, cache_position=cache_position)
...
class Zamba2Model(Zamba2PreTrainedModel):
    ...
    def __init__(self, config: Zamba2Config):
        ...
        self.layers = [SimpleNamespace(mixer=SimpleNamespace(M={})) for _ in self.layers_block_type]
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        knock_indices: Optional[torch.LongTensor] = None,
        ...
        for layer_idx, layer_type in enumerate(self.layers_block_type):
            ...
            if layer_type == "g":
                ...
                if getattr(self.layers[layer_idx].mixer, "materialize_heads", []) and layer_outputs[1] is not None:
                    self.layers[layer_idx].mixer.M = {(layer_idx, h): layer_outputs[1][0][h] for h in self.layers[layer_idx].mixer.materialize_heads}
                else:
                    self.M = {}
                block_count += 1
...
class Zamba2ForCausalLM(Zamba2PreTrainedModel):
    ...
    def forward(
        ...
        ```"""
        if False:
            ends = torch.isin(input_ids, input_ids[:, (input_ids != 0).sum(dim=1) - 1]).nonzero()
        else:   
            # Rolling window to find sub-tensor in each row
            sub_tensor = torch.tensor([13, 13], device=input_ids.device) # corresponds to \n\n
            sliding_windows = input_ids.unfold(dimension=1, size=sub_tensor.shape[0], step=1)  # Shape: [batch, seq_len - sub_len + 1, sub_len]
            matches = (sliding_windows == sub_tensor).all(dim=-1)  # Shape: [batch, seq_len - sub_len + 1]
            ends = matches.nonzero(as_tuple=False)  # [batch_idx, start_idx]

            # If it is MMLU, we need to make sure no extras are included (MMLU has \n\n beyond few-shot examples)
            if (input_ids[0, :6] == torch.Tensor([415, 2296, 460, 5166, 4782, 4224]).to(input_ids.device)).all(): 
                abcd = torch.tensor([330, 365, 334, 384], device=input_ids.device)
                previous_tokens = input_ids[ends[:, 0], ends[:, 1] - 1]
                mask = torch.isin(previous_tokens, abcd).nonzero()
                ends = ends[mask].squeeze(1)

            # ends[:, 1] -= 2

            # Assert that each example has exactly 5 newlines (5 shots)
            from collections import Counter
            counts = list(Counter(ends[:, 0].tolist()).values())
            assert len(counts) == 0 or max(counts) == min(counts)
        

        # Save
        for block in self.model.blocks:
            block.self_attn.heads = range(32)
            block.self_attn.ends = ends
