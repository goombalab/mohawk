...
def test_docs_scope_default_and_ssm_requirements_installs():
    ...
    assert "configs/smoke/" not in readme
    assert "/home/abick/" not in readme
    assert len(readme.splitlines()) <= 250
    assert len(contributing.splitlines()) <= 150


def test_public_docs_show_runnable_evaluation_commands():
    readme = (ROOT / "README.md").read_text()

    assert "python evals/benchmark.py" in readme
    assert "python evals/eval_ppl.py" in readme
    assert "python generation/generate.py" in readme
    assert "--local_files_only" in readme
