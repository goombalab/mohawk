...
from utils.distributed import get_device, world_size
...
def distill(cfg):
    ...
    try:
        ...
        for step in range(scheduler._step_count, scheduler.num_steps):
            ...
            # Record time & score
            duration = torch.tensor(time.time() - start).to(device)
            avg_score = torch.tensor(scores).mean().to(device)
        ...
        saved_dir = student_wrapper.save_weights()
        logger.info(f'[TRAIN] Training finished.\nWeights saved in {saved_dir}')
    finally:
        if dataloader_iter is not None:
            close = getattr(dataloader_iter, "close", None)
            if close is not None:
                close()
        if train_dataloader is not None:
            close = getattr(train_dataloader, "close", None)
            if close is not None:
                close()
        evaluator = None
        train_dataloader = None
