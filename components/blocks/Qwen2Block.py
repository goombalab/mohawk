...
class Block(nn.Module):
    def __init__(self, d_model, config, factory_kwargs, layer_idx, **kwargs):
        ...
        self.input_layernorm = Qwen2RMSNorm(hidden_size=self.d_model, eps=norm_epsilon)
        self.post_attention_layernorm = Qwen2RMSNorm(hidden_size=self.d_model, eps=norm_epsilon)
        self.mlp = Qwen2MLP(
            Qwen2Config(
