...
from utils.logging import logger as eval_logger
...
class HFLM(TemplateLM):
    ...
    def _loglikelihood_tokens(
        ...
        chunks_len = len(list(re_ord.get_batched(n=batch_size, batch_fn=batch_fn)))
        max_chunks_len = torch.tensor(chunks_len, device=self.device)
        if self._world_size > 1:
            torch.distributed.all_reduce(max_chunks_len, op=torch.distributed.ReduceOp.MAX)
        ...
        for chunk in chunks:
            ...
            assert not torch.isnan(multi_logits).any(), "multi_logits contains NaNs"
        ...
        for _ in range(max_chunks_len - chunks_len):
            # use previously computed batched_inps and continue
    ...
    def generate_until(
        ...
        # check that all ranks have the same number of chunks
        chunks_len = len(list(re_ords.get_batched(n=batch_size, batch_fn=batch_fn)))
        max_chunks_len = torch.tensor(chunks_len, device=self.device)
        if self._world_size > 1:
            torch.distributed.all_reduce(max_chunks_len, op=torch.distributed.ReduceOp.MAX)
